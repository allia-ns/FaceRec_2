...
Face Recognition GUI using Streamlit - FIXED VERSION
"""
...
# Safe import with proper error handling (like in FaceRec_Gui.py)
...
def show_import_status():
    """Show the import status and try to import"""
...
def load_and_cache_model():
    """Load model with caching - FIXED VERSION"""
    ...
    if not st.session_state.model_loaded:
        with st.spinner("📥 Loading trained model..."):
            try:
                ...
                if mean_face is not None:  # Check if model loaded successfully
                    # Store ALL model data in session state (like FaceRec_Gui.py)
...
def perform_recognition(uploaded_file, threshold=15.0):
    """Perform face recognition - FIXED VERSION"""
    ...
    try:
        with st.spinner("🔍 Analyzing face..."):
            # Use session state data (like FaceRec_Gui.py)
...
def display_distance_analysis(uploaded_file, threshold=15.0):
    """Display distance analysis and visualization - FIXED"""
    if not st.session_state.get('model_loaded', False) or uploaded_file is None:
        return
...
def main():
    ...
    # Show import status and stop if failed
    if not show_import_status():
        st.stop()
